 ...
 * subject to an additional IP rights grant found at
 * http://polymer.github.io/PATENTS.txt
 */const w=` ${o} `;class S{constructor(e,t,n,r){this.strings=e,this.values=t,this.type=n,this.processor=r}getHTML(){const e=this.strings.length-1;let t="",n=!1;for(let r=0;r<e;r++){const e=this.strings[r],i=e.lastIndexOf("\x3c!--");n=(i>-1||n)&&-1===e.indexOf("--\x3e",i+1);const a=h.exec(e);t+=null===a?e+(n?w:s):e.substr(0,a.index)+a[1]+a[2]+"$lit$"+a[3]+o}return t+=this.strings[e],t}getTemplateElement(){const e=document.createElement("template");return e.innerHTML=this.getHTML(),e}}
/**
 * @license
 * Copyright (c) 2017 The Polymer Project Authors. All rights reserved.
 * This code may only be used under the BSD style license found at
...
.fc-rtl .fc-time-grid .fc-now-indicator-arrow {
  ...
  border-top-color: transparent;
  border-bottom-color: transparent;
}`;class Ts{constructor(e){this.title=e.summary?e.summary:e.title,this.start=e.start.date?e.start.date:e.start.dateTime?e.start.dateTime:e.start,this.end=e.end.date?e.end.date:e.end.dateTime?e.end.dateTime:e.end,this.allDay=!!e.start.date}}class Cs{constructor(){this.eventData={}}async getCalendarEvents(e,t,n,r){if(!e)return[];let i=n.toISOString(),o=r.toISOString(),s=`calendars/${t.entity}?start=${i}&end=${o}`;var a=(await e.callApi("get",s)).map(e=>new Ts(e));let l=this.eventData[t.entity]||[];return l.push(...a),this.eventData[t.entity]=l.filter((e,t)=>{const n=JSON.stringify(e);return t===l.findIndex(e=>JSON.stringify(e)===n)}),this.eventData[t.entity]}}customElements.define("fullcalendar-card",class extends ee{setConfig(e){if(!e||!e.entities)throw new Error("Invalid configuration");this._config=e,this.entities=this.processConfigEntities(this._config.entities),this.calendarService=new Cs}createRenderRoot(){return this}get hostcss(){return this.isPanel?O`<style type="text/css">
			:host #calendar {
        		width: 100%;
        		height: 100%;
